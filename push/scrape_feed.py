...
#  配置
# ============================================================
TARGET_POSTS_PER_TAB = 15   # quick mode: 每个 tab 抓取目标条数
MAX_SCROLLS = 12            # quick mode: 每个 tab 最多滚动次数
WAIT_SECONDS = 2            # 每次滚动后等待秒数
SCRAPE_TRENDING = False     # quick mode: 不爬 Trending，只抓帖子
TRENDING_TOPIC_COUNT = 0    # quick mode: 不需要

# 健壮性配置
MAX_RETRIES = 2             # 页面加载重试次数
RETRY_DELAY = 3             # 重试间隔秒数
STALE_SCROLL_LIMIT = 3      # 连续多少次滚动无新帖则提前退出
GLOBAL_TIMEOUT = 180         # 全局超时秒数（3分钟，quick mode 足够）
